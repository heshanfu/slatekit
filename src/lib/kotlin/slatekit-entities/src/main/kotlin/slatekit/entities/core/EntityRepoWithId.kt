 ...
 * @tparam T
 */
abstract class EntityRepoWithId<TId, TEntity>(
    entityType: KClass<*>,
    ...
    : IEntityRepo where TEntity : EntityWithKey<TId> {
     ...
     * @return
     */
    abstract fun create(entity: TEntity): TId
     ...
     * @return
     */
    abstract fun update(entity: TEntity): TEntity
     ...
     * @return
     */
    abstract fun get(id: Long): TEntity?
     ...
     * @return
     */
    abstract fun get(ids: List<Long>): List<TEntity>

    /**
     * gets all the entities from the datastore.
     * @return
     */
    abstract fun getAll(): List<TEntity>
     ...
     * @return
     */
    abstract fun top(count: Int, desc: Boolean): List<TEntity>
     ...
     * @param items
     */
    fun saveAll(items: List<TEntity>) = items.forEach { item -> save(item) }

    /**
     * Gets the first/oldest item
     * @return
     */
    fun first(): TEntity? = takeFirst { oldest(1) }

    /**
     * Gets the last/recent item
     * @return
     */
    fun last(): TEntity? = takeFirst { recent(1) }
     ...
     * @return
     */
    fun recent(count: Int): List<TEntity> = top(count, true)
     ...
     * @return
     */
    fun oldest(count: Int): List<TEntity> = top(count, false)
     ...
     * @return
     */
    fun takeFirst(call: () -> List<TEntity>): TEntity? = call().firstOrNull()
     ...
     * @return
     */
    open fun find(query: IQuery): List<TEntity> = listOf()
     ...
     * @return
     */
    open fun findBy(field: String, op: String, value: Any): List<TEntity> = listOf()
     ...
     * @return
     */
    open fun findIn(field: String, value: List<Any>): List<TEntity> = listOf()
     ...
     * @return
     */
    open fun findFirstBy(field: String, op: String, value: Any): TEntity? = null
     ...
     * @return
     */
    open fun findByProc(name: String, args: List<Any>?): List<TEntity>? = listOf()

    /**
     * Hook for derived classes to handle additional logic before saving
     * @param entity
     */
    protected fun onBeforeSave(entity: TEntity) = {}
}
